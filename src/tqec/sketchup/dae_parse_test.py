...
def _group_nodes_by_type(nodes: list[Node]) -> dict[str, set[tuple[int, int, int]]]:
    nodes_by_type: dict[str, set[tuple[int, int, int]]] = {}
    for node in nodes:
        nodes_by_type.setdefault(node.node_type, set()).add(node.position.to_tuple())
