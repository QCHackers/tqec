...
def _find_cover(
    ...
    def manhattan_distance_from_target(source: BoundaryStabilizer) -> float:
    ...
    print(f">>> Before spatial filtering: {len(sources)}")
    sources = [
        s
        for s in sources
        if manhattan_distance_from_target(s) <= maximum_qubit_distance
    ]
    print(f">>> After spatial filtering: {len(sources)}")
...
def _match_boundary_stabilizers_by_disjoint_cover(
    ...
    for i, target in enumerate(target_stabilizers):
        cover = _find_cover(target, covering_stabilizers, qubit_coordinates)
        if cover is None:
            continue
