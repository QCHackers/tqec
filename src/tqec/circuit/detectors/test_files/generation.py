...
VALID_TEST = TEST_DIRECTORY / "valid"
...
for code_task, distance, rounds in itertools.product(
    code_tasks, (3, 5), (1, 2, 13, 100)
):
    relative_filepath = (
        code_task.replace(":", "_") + f"_distance_{distance}_rounds_{rounds}.stim"
    )
    ...
    circuit = reorder_resets(split_combined_measurement_reset(circuit))
    with open(VALID_TEST / relative_filepath, "w") as f:
        f.write(str(circuit))
