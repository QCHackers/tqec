...
class RPNG:
    ...
    def from_string(cls, rpng_string: str) -> RPNG:
        ...
        if p and not a:
            raise ValueError(
                "Unacceptable ancilla basis in the extended format (i.e. rapng)."
            )
        if not n and n_str != "-":
    ...
    def __post_init__(self):
        ...
        for rpng in self.corners:
            if rpng.n:
                ...
                times.append(rpng.n)
            # Confirm that at least one between fields a and p in every corner is z, if 2Q gate is present
            if rpng.p and rpng.a != BasisEnum.Z and rpng.p != BasisEnum.Z:
                raise ValueError(
                    'At least one between a and p must be "z" in every corner.'
                )
        ...
        for q, rpng in enumerate(self.corners):
            ...
            if rpng.n and rpng.p:
                if rpng.a.value == "z":
                    ...
                    circuit_as_list[rpng.n] += (
                        f"{rpng.a.value.upper()}C{rpng.p.value.upper()} 4 {q}\n"
                    )
